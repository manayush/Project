...
class DQN():
    def __init__(self, gym_game, epsilon=1, epsilon_decay=0.99, epsilon_min=0.01, batch_size=32, discount_factor=0.9, num_of_episodes=500):
    ...
    def create_model(self):
        ...
        try:
            ...
            input = Input(shape=(self.state_size,1))

        if self.state_mode == "information":
            # if the state is not an image
            out = Conv1D(16, kernel_size=8, padding="SAME", activation="relu")(input)
            out = Flatten()(out)
            out = Dense(12, activation="relu")(out)
            value = Dense(8, activation="relu")(out)
            value = Dense(1, activation="relu")(value)
            advantage = Dense(8, activation="relu")(out)
            advantage = Dense(self.action_size, activation="relu")(advantage)
            advantage_mean = Lambda(lambda x: tf.keras.backend.mean(x, axis=1), output_shape=(self.action_size,))(advantage)
            ...
            out = MaxPooling2D()(out)
            out = Flatten()(out)
            value = Dense(64, activation="relu")(out)
            value = Dense(1, activation="relu")(value)
            advantage = Dense(64, activation="relu")(out)
            advantage = Dense(self.action_size, activation="relu")(advantage)
    ...
    def save_info(self, episode, reward, time):
        file = open("./Plot/DuelingDQN-"+self.game+"-"+str(self.num_of_episodes)+"-episodes-batchsize-"+str(self.batch_size), 'a')
...
#game = "Pong-v0" # bd
game = "CartPole-v1" # bd
dqn = DQN(game, num_of_episodes=1000)
dqn.play()